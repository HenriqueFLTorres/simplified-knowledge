...
<Link to={`/blog/${attributes.alternativeLanguageURL}`}><div>Artigo em Português</div></Link>

<img src={mainImage} alt="" title="Photo by David Jorre" className="MainImage" />

<Warning>In the following examples emojis are used to name variables and should not be replicated, otherwise your code will return a syntax error.</Warning>
...
So knowing the difference between <Highlight>reference</Highlight> and actual <Highlight>value</Highlight> is critical for learning advanced hooks in React like <Code>useEffect()</Code>, <Code>useCallback() </Code> and <Code>useMemo()</Code>.
...
Another example is the <Code>useCallback()</Code>, used to memorize functions and not just compare with their reference, preventing the creation of another identical function on every rendering. If you want a post talking about it, let me know!
