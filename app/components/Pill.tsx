...
const Pill = ({ name, icon, handleSelectedTags, activeTags }: Props) => {
  ...
  return (
    <div onClick={() => handleSelectedTags(name)} className={`group select-none flex flex-row justify-between border ${active ? "border-neutral-800 dark:bg-neutral-200/[0.15] dark:border-neutral-200" : "bg-neutral-200 dark:bg-neutral-800 border-transparent" } p-2.5 rounded hover:border-neutral-700 dark:hover:border-neutral-500 transtion-all duration-150 cursor-pointer`}>
      ...
      <p className={`${active ? "text-neutral-800 dark:text-neutral-200" : "text-neutral-400"}  text-xl font-regular mx-2 group-hover:text-neutral-800 dark:group-hover:text-neutral-100 transition-all duration-150`}>{name}</p>
    </div>
  );
};
